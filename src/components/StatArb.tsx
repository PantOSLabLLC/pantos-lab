...
// Sample correlation pair: Asset A vs Asset B returns (%). Cointegrated pair → points along diagonal
const PAIR_DATA = [
  { x: -1.2, y: -1.1 },
  { x: 0.8, y: 0.7 },
  { x: -0.5, y: -0.6 },
...
function CorrelationPairChart() {
  ...
  // Regression line: y ≈ x (high correlation)
  ...
  return (
    <div className="panel p-6 rounded-2xl" ref={containerRef}>
      <div className="h-[220px] lg:h-[240px] relative">
        <svg
          ref={svgRef}
...
export function StatArb() {
  const ref = useRef<HTMLElement>(null)

  const visual = <CorrelationPairChart />
    ...
    <StaggeredContent
      ...
      title={
        <h2 className="font-heading text-[clamp(1.75rem,3vw,2.5rem)] font-medium tracking-tight text-white mb-6">
          Pairs Screening & Long/Short
        </h2>
      }
    >
      <p className="text-xl text-white/70 mb-8 leading-relaxed">
        Software for screening and trading pairs of stocks for long/short strategies.
      </p>
      <ul className="space-y-3">
        {[
          'Tools for screening and trading pairs of stocks (for long / short trading)',
        ].map((item) => (
  ...
  return (
    <section id="statarb" ref={ref} className="min-h-[75vh] flex items-center py-20 px-8 lg:px-16">
